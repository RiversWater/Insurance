...
mLayer.prototype.render = function() {
    ...
    if (typeof settings.content == "object") {
        layerBox.querySelector(".layer-content").innerHTML = "";
        let cloneNode = settings.content[0];
        cloneNode.style.display = "block"

        layerBox.querySelector(".layer-content").appendChild(cloneNode)
    }
...
mLayer.prototype.action = function() {
    ...
    if (settings.time) {
        timer[this.index] = setTimeout(that.close, settings.time, this);
    }
...
mLayer.prototype.close = function(layer) {
